...
#### Works `order` フィールドの設計方針

`order` は **降順（大きい値が Featured として前に出る）** で運用する。

- 新しい作品を追加するたびに既存のすべての値を書き換えなくて済むよう、将来の追加分に大きな数値を割り当てられる降順を採用している。
- 昇順（`1` がトップ）にすると、常に `order: 1` を1つしか持てず、追加のたびに全エントリを振り直す必要が生じる（数値インフレ対策）。
- 同じ `order` 値を持つエントリは **`period` の降順（新しい順）** を二次キーとして並べる。
